...
def _load_dataset(data_files, extension='json', 
        test_split_percentage=10, min_tokens=5, 
        download_mode='reuse_dataset_if_exists', num_proc=1):
    ...
    for name in dataset:
        ...
        dataset[name] = dataset[name].filter(lambda x: len(x['text'].split()) > min_tokens, num_proc=n_proc)
        # print(dataset[name]['text'][0])
        print('Loading Datasets: There are', len(dataset[name]), f'records in {name} dataset.', data_files)
    return dataset

class ShardingDataset:
    def __init__(self, corpus,
        output, dataset, n_shards, 
        n_processes, segmenter, **args):
        ...
        if corpus_files:
            ...
            elif isinstance(corpus_files, list) or type(corpus_files) == omegaconf.listconfig.ListConfig:
            ...
            corpus_files = {str(f):1 for f in Path(corpus_dir).glob('**/*') if f.is_file()}
        assert len(corpus_files) > 0, 'The input file list must contain at least one file.'
        ...
        self.output_files = {'train': {}, 'test': {}}    # key: filename, value: list of articles to go into file
    ...
    def load_datasets(self):
        ...
        for input_file, dupe_factor in self.input_files.items():
            ...
            dset = _load_dataset(input_file, extension=self.input_extension, 
                    test_split_percentage=int(self.fraction_test_set*100), 
                    min_tokens=self.min_tokens, download_mode=self.download_mode,
    ...
    def segment_articles_into_sentences(self):
        ...
        assert len(self.datasets['train']) > 0, 'Please check that input files are present and contain data.'
        assert self.segmenter['_target_'], 'Please check that the segmenter is configured correctly.'
        ...
        for name in self.datasets:
            ...
            self.datasets[name] = self.datasets[name].map(segment_into_sentences, batched=True, num_proc=num_proc)
            num_sentences += len(self.datasets[name])
            
        print(f'Number of articles: {num_articles}, Number of sentences: {num_sentences}')
    ...
    def init_output_files(self):
        ...
        for name in self.output_files:
            assert len(self.output_files[name]) is 0, 'Internal storage self.output_files already contains data. This function is intended to be used by the constructor only.'
        
            for i in range(self.n_shards[name]):
                filename = self.output_name_prefix + self.output_identifier[name] + '_' + str(i) + self.output_file_extension
    ...
    def distribute_datasets_over_shards(self):
        ...
        for name in self.datasets:
            assert len(self.datasets[name]) > self.n_shards[name], 'There are fewer articles than shards. Please add more data or reduce the number of shards requested.'

            for i, filename in enumerate(self.output_files[name]):
                self.output_files[name][filename] = self.datasets[name].shard(self.n_shards[name], i, contiguous=True)
    ...
    def write_shards_to_disk(self):
        ...
        for split, n in self.n_shards.items():
            if n > 0:
                ...
                command = 'mv ' + absolute_dir + '/*' + split + '*.txt' + ' ' + absolute_dir + '/' + split
                print(command)
