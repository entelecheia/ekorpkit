...
logging.basicConfig(
    format="[MeCab]: %(message)s", level=logging.WARNING
)
...
class MeCab:  # APIs are inspried by KoNLPy
    def __init__(self, dicdir=None, userdic_path=None, 
            backend='mecab-python3', verbose=False,
            **kwargs):
        import mecab_ko_dic
        ...
        if self.verbose:
            logging.info(f'Mecab uses system dictionary: {dicdir}, user dictionary: {userdic_path}')
        try:
            ...
            raise Exception('The MeCab dictionary does not exist at "%s". Is the dictionary correctly installed?\nYou can also try entering the dictionary path when initializing the MeCab class: "MeCab(\'/some/dic/path\')"' % self.dicdir)
        except NameError:
    ...
    def morphs(self, sentence, flatten=True, include_whitespace_token=True):
        return [
            surface 
            for surface, _ in self.pos(sentence, flatten=flatten, join=False, include_whitespace_token=include_whitespace_token)
        ]

    def nouns(self, sentence, flatten=True, include_whitespace_token=True, noun_pos=['NNG', 'NNP', 'XSN', 'SL', 'XR', 'NNB', 'NR']):
        return [
            surface 
            for surface, pos in self.pos(sentence, flatten=flatten, join=False, include_whitespace_token=include_whitespace_token)
            if pos in noun_pos
        ]
...
ContextEntry = namedtuple('ContextEntry', [
    'id',
    'pos',
    'semantic',
    'has_jongseong',
...
class MecabDicConfig:
    ...
    def find_right_context_id(self, search):
        for entry in self.right_ids:
            if entry.pos == search.pos and entry.semantic == search.semantic and entry.has_jongseong == search.has_jongseong:
                return entry.id
    ...
    def add_entry_to_userdic(self, surface, pos='NNP', 
                             semantic='*', reading=None, cost=1000):
        entry = DicEntry(
                    surface=surface, 
                    cost=cost,
                    pos=pos, 
    ...
    def build_userdic(self, built_userdic_path, userdic_path=None):
        if userdic_path:
            self.userdic_path = userdic_path
        args = '-d "{}" -u "{}" {}'.format(self.dicdir, built_userdic_path, self.userdic_path)
        # print(args)
