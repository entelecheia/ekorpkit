...
class MPTokenizer:
    ...
    def tokenize_texts(self, input_path=None, output_path=None, **args):
        ...
        for worker_id in range(num_workers):
            pool.apply_async(
                worker_function,
                (input_path, output_path, worker_id, self.backend, self.kwargs, offsets[worker_id], offsets[worker_id+1])
            )
        pool.close()
    ...
    def tokenize_dataframe(self, input_dataframe=None, text_key='text', **args):
        ...
        split_dfs = [input_dataframe.iloc[offsets[worker_id]:offsets[worker_id+1], :] 
                        for worker_id in range(num_workers)]
        ...
        for worker_id in range(num_workers):
            results.append(
                pool.apply_async(
                    ...
                    (split_dfs[worker_id], text_key, worker_id, self.backend, self.kwargs, offsets[worker_id], offsets[worker_id+1])
                )
            )
        pool.close()
        logger.info("[Complete] join all workers")
...
class Tokenizer():
    def __init__(self, backend='pynori', wordpieces_prefix=None, 
            tokenize_each_word=False, normalize=False, **kwargs):
    ...
    def tokenize(self, text):
        ...
        if self.backend == "pynori":
            ...
            term_pos = [f'{term}/{pos}' for term, pos in zip(tokens['termAtt'],tokens['posTagAtt'])]
...
def worker_function(input_path, output_path, worker_id, backend, kwargs, start_offset=0, end_offset=-1):
...
def worker_function_for_df(df, text_key, worker_id, backend, kwargs, start_offset=0, end_offset=-1):
    """ 단일 프로세스에 사용되는 worker 함수 """
    logger.info(f"[Start] worker_id: {worker_id} [start_offset: {start_offset} & end_offset: {end_offset}]")
    _tokenizer = Tokenizer(backend=backend, **kwargs)
...
def extract_tokens(text, 
    ...
    **kwargs
    ...
    if nouns_only:
        tokens = [token[0].strip() for token in _tokens 
                    if token[1] in noun_pos_filter]
    else:
        exist_sp_tag = False
        ...
        if exist_sp_tag and no_space_for_non_nouns:
            ...
            while i < len(_tokens):
                ...
                elif not prev_nonnoun_check and token[1] not in noun_pos_filter and token[1][0] != 'S':
                    prev_nonnoun_check = True
                    cont_morphs.append(token[0])
                elif prev_nonnoun_check and token[1] not in noun_pos_filter and token[1][0] != 'S':
                    cont_morphs.append(token[0])
                else:
