...
def normalize(
    doc, english=False, number=False, punctuation=False, remove_repeat=0, remains={}
):
    ...
    for c in doc:
        ...
        if (number) and (i >= 48 and i <= 57):
            ...
            continue
        if (punctuation) and (
            i == 33 or i == 34 or i == 39 or i == 44 or i == 46 or i == 63 or i == 96
        ):
            f.append(c)
            continue
        if c in remains:
        ...
        for c in encoded_sent:
            if len(c) == 1:
                if not 0 <= c[0] < self.dim:
                    raise ValueError(
                        "character index %d is out of index [0, %d]" % (c[0], self.dim)
                    )
