...
class Segmenter:
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        self.in_segment_separator = codecs.decode(in_segment_separator, 'unicode_escape')
        self.in_sentence_separator = codecs.decode(in_sentence_separator, 'unicode_escape')
        self.out_segment_separator = codecs.decode(out_segment_separator, 'unicode_escape')
        self.out_sentence_separator = codecs.decode(out_sentence_separator, 'unicode_escape')
        self.return_type = return_type
        self.max_split_length = max_split_length
        self.max_split_iterations = max_split_iterations
        self.keep_segment = keep_segment
        self.merge_lines = merge_lines if isinstance(merge_lines, bool) else str(merge_lines).lower() in ['true', 't', '1']
        ...
        self.kwargs = kwargs
        if print_args:
            print(f'{self.in_segment_separator = }')
            print(f'{self.in_sentence_separator = }')
            print(f'{self.out_segment_separator = }')
            print(f'{self.out_sentence_separator = }')
            print(f'{self.return_type = }')
            print(f'{self.max_split_length = }')
            print(f'{self.max_split_iterations = }')
            print(f'{self.keep_segment = }')
            print(f'{self.merge_lines = }')
            print(f'{self.merge_level = }')
            print(f'{self.empty_lines_threshold = }')
            print(f'{self.broken_lines_threshold = }')
            print(f'{self.filter_language = }')
            print(f'{self.detection_level = }')
            print(f'{self.languages_to_keep = }')
            print(f'{self.min_language_probability = }')
            print(f'{self.filter_programming_language = }')
    ...
    def _check_if_merge_lines(self, text):
        ...
        broken_lines_ratio = 1.0 - num_lines_endswith_period / (num_lines-num_empty_lines) if (num_lines-num_empty_lines) > 0 else 0
        if self.verbose:
    ...
    def segment_article(self, article):
        ...
        article = re.sub(f'{self.in_segment_separator}+', self.in_segment_separator, article)
        ...
        return segments if self.return_type == 'list' else self.out_segment_separator.join(segments)
    ...
    def _skip_language(self, text):
        ...
        if lang is not None and lang['lang'] not in self.languages_to_keep and lang['score'] < self.min_language_probability:
            if self.verbose > 5:
...
class PySBDSegmenter(Segmenter):
    def __init__(self, language="en", clean=False, doc_type=None,
        **kwargs):
        ...
        if doc_type == 'pdf':
            self.clean = True
        self.seg = pysbd.Segmenter(language=self.language, clean=self.clean, doc_type=self.doc_type)
        super().__init__(**kwargs)
