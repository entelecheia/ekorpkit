...
class Batcher(object):
	...
	def process_batches(self, task, data, args, backend=None, backend_handle=None, input_split=False,
						...
						task_num_gpus= None, verbose= None, description='batch_apply'):
		"""
		...
		if verbose > 1:
			print(" backend:", backend, " minibatch_size:", self.minibatch_size, " procs:", procs,
		      " input_split:", input_split, " merge_output:", merge_output,
			  " len(data):", len(data), "len(args):", len(args))
		...
		if verbose > 10:  print(" Start task, len(paral_params)", len(paral_params))
		if backend == "serial":
			results = [task(minibatch) for minibatch in tqdm(paral_params, desc=description)]
		else:
			if backend=="multiprocessing":
				...
				with tqdm_joblib(tqdm(desc=description, total=len(paral_params))) as pbar:
					results= Parallel(n_jobs=procs)(delayed(task)(params) for params in paral_params)
			elif backend=="p_tqdm":
				from p_tqdm import p_map
				results= p_map(task, paral_params, num_cpus=procs)
			elif backend == "dask":
				###if not (input_split):  data= self.scatter(data)
				results = [self.backend_handle.submit(task, params) for params in tqdm(paral_params, desc=description)]
			elif backend == "spark":
