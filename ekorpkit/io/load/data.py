...
def load_data(split_name, **loader_cfg):
	...
	if processes > 2 and multiprocessing_at == 'load_data':
		print(f'Starting multiprocessing with {processes} processes at {multiprocessing_at}')
		...
		for fno, filepath in enumerate(filepaths):
			...
			documents += _load_archive(filepath, filetype, default_items, loader_cfg, num_workers)
...
def _load_archive(filepath, filetype, default_items, loader_args, num_workers):
	...
	num_files = len(files)
	if num_workers > 2 and multiprocessing_at == '_load_archive' and num_files > num_workers:
		print(f'Starting multiprocessing with {num_workers} processes at {multiprocessing_at}')
	...
	for fno, (file, filename) in enumerate(files):
		...
		if executor is not None:
			...
			results.append(executor.submit(parse_data, contents, loader_args, default_items, 1))
			pbar.update(1)
		else:
...
def load_hfds(split_name, **loader_cfg):
	...
	if isinstance(subsets, str):
		...
		subsets = [None]
	print(f'Loading [{split_name}] documents from huggingface datasets [{dataset_name}]')
	dfs = []
		...
		if filepath.endswith('.zip'):
			with ZipFile(filepath) as zf:
				for f_csv in zf.namelist():
					with zf.open(f_csv, 'r') as zfo:
						content = zfo.read().decode(errors='ignore')
			df = pd.read_csv(StringIO(content), index_col=None, sep=sep, dtype=dtype, names=names)
		else:
			df = pd.read_csv(filepath, index_col=None, sep=sep, dtype=dtype, names=names)
		documents.append(df)
	df = pd.concat(documents, ignore_index=True)
...
def read_conll(filename, **args):
    ...
    names =['words', 'pos', 'chunk', 'labels'] if column_dtypes is None else column_dtypes.keys()
