...
class SecReport:
	...
	def __init__(self, name, output_dir, output_file, 
			limit=0, mongo=None, debug_mode=False, 
			num_workers=1, normalize=None,
	...
	def _normalize_text(self, text):
		...
		for paragraph in re.split(r'\n\n', text):
			...
			for s in re.split(r'\n', paragraph):
				...
				if len(s) >= self.min_len and len(s.split()) >= self.min_words and not skip:
	...
	def _preprocess_report(self, rst):
		...
		doc = {'rid': rid, 'rdate': rdate,
			'type': rpt_typ, 'subtype': rpt_sub_typ,
			'nos': nos,'text': text}
		return doc
