...
class BOKMniutes():
	...
	def build_minutes(self):	
		...
		df = pd.DataFrame(docs, columns=['id', 'filename', 'mdate', 'rdate', 'section', 'text'])
	...
	def get_minutes_files(self):
		for info in self.minutes_urls:
			self.get_minutes_file(info['url'], info['meeting_date'], info['release_date'])

	def get_minutes_file(self, page_addr, mdate, rdate):
		filename = self.file_prefix + "_"  + mdate.strftime('%Y%m%d') + "_" + rdate.strftime('%Y%m%d')+ '.hwp'
		...
		for link in links:
			...
			link_filename = link_filename.replace('\r', '').replace('\t', '').replace('\n', '')
	...
	def get_minutes_list(self):
		...
		for post in brdList:
			...
			info = {
				'meeting_date': mdate, 'release_date': rdate, 
				'url': guid, 'title': title
			}
...
def preprocess_minutes(filepath, file_prefix):
	...
	minutes = open(filepath, encoding=u'utf-8').read()
	...
	section_texts = (section1_txt, section2_txt, section3_txt, section4_txt, section5_txt, section6_txt)
