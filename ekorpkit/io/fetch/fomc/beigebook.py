...
class BeigeBook(FomcBase):
	...
	def __init__(self, content_type, **args):
		...
		self.beige_book_url = 'https://www.federalreserve.gov/monetarypolicy/beige-book-default.htm'
	...
	def _get_links(self, from_year):
		...
		if self.verbose: print("Getting links for Beigebooks...")
		contents = soup.find_all('a', href=re.compile('^/monetarypolicy/beigebook\d{6}.htm'))
		dates = soup.find_all('a', href=re.compile(self.base_url + '/monetarypolicy/files/BeigeBook_\d{8}.pdf'))
		dates = [content.attrs['href'] for content in dates]
		self.links = [content.attrs['href'] for content in contents]
		self.speakers = [self._speaker_from_date(self._date_from_link(x)) for x in dates]
		self.titles = ['FOMC Beige Book'] * len(self.links)
		self.dates = [datetime.strptime(self._date_from_link(x), '%Y-%m-%d') for x in dates]
		...
		if from_year <= to_year:
			...
			for year in range(from_year, to_year+1):
				yearly_contents = []
				beige_book_annual_url = self.base_url + '/monetarypolicy/beigebook' + str(year) + '.htm'
				...
				for yearly_content, yearly_date in zip(yearly_contents, yearly_dates):
					...
					self.speakers.append(self._speaker_from_date(self._date_from_link(yearly_date)))
					self.titles.append('FOMC Beige Book')
					self.dates.append(datetime.strptime(self._date_from_link(yearly_date), '%Y-%m-%d'))
					# Correct some date in the link does not match with the meeting date
