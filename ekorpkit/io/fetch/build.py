...
class DatasetBuilder:
	...
	def _load_info(self):
		self.info_path = Path(self.data_dir) / self.info_args.get('info_file', f'info-{self.name}.yaml')
	...
	def _update_info(self, split_infos):
		...
		for key, value in self.info_args.modified_info.items():
			vals = [get_modified_time(f'{self.data_dir}/{split[value]}') 
						for split in self.info['splits'].values() if value in split]
	...
	def _process_split(self, split_name):
		...
		output_meta_file = output_dir / f'meta-{self.name}-{split_name}{self.data_filetype}'
		...
		if not output_file.exists() or self.overwrite:
			...
			if stat_args:
				with elapsed_timer(format_time=True) as elapsed:
					...
					msg.good(f' >> elapsed time to calculate statistics before processing: {elapsed()}')
					if verbose:
						pprint(stat_info)
