...
def download_file_full(url, local_file, content_length):
    try:
        ...
        headers = {"Accept-Encoding": "identity"}  # Avoid dealing with gzip
        with tqdm(
            total=content_length, unit="byte", unit_scale=1
        ) as progress, session.get(
...
def download_file(
    urls, expected_checksum=None, local_file=None, local_directory=None, max_retries=3
):
    ...
    try:
        for url in urls:
            ...
            logger.info(
                f"Accept-Ranges: {accept_ranges}. content length: {content_length}"
            )
            ...
            for i in range(max_retries):
                ...
                if checksum:
                    ...
                    if expected_checksum and expected_checksum != checksum:
                        logger.info(
                            f"Checksum doesn't match. Calculated {checksum} Expecting: {expected_checksum}"
                        )
                    else:
