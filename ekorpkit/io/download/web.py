...
def web_download(url, output_path, name='', force_download=False):
    ...
    if (not force_download) and os.path.exists(output_path) and (os.stat(output_path).st_size == remote_size):
        print(f'[eKorpkit] Corpus `{name}` is already downloaded at {output_path}')
        return None
    filename = os.path.basename(output_path)
    with tqdm(unit='B', unit_scale=True, miniters=1, desc=f'[{name}] download {filename}') as t:
