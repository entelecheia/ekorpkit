...
def convert_electra_ckpt_to_pytorch(**cfg):
	...
	if args.checkpoint_files:
		...
		checkpoint_files = [Path(f).stem for f in glob.glob(f'{args.checkpoints_dir}/*.index')]
	...
	for ckpt in checkpoint_files:
		...
		for model_type in ['discriminator', 'generator']:
			...
			convert_tf_checkpoint_to_pytorch(
				args.checkpoints_dir, config_file, pytorch_dump_path, model_type, args.load_weights_func._target_
			)
