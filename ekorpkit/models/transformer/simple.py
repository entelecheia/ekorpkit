...
class SimpleTraner():
	def __init__(self, **args):
		...
		wandb.init(config=args, group=args.wandb_group, project=args.wandb_project, dir=args.wandb_dir)
...
class SimpleTrainerNER(SimpleTraner):
	...
	def train(self):
		...
		# Create a NERModel
		model = NERModel(args.model_type, args.model_uri,
						labels=labels,  
...
class SimpleTrainerMultiLabel(SimpleTraner):
	...
	def train(self):
		...
		args = self.args
		# Create a Model
		model = MultiLabelClassificationModel(args.model_type, args.model_uri,
...
class SimpleTrainerClassification(SimpleTraner):
	...
	def train(self):
		...
		# Create a NERModel
		model = ClassificationModel(args.model_type, args.model_uri,
						num_labels=args.num_labels, 
						cuda_device=args.cuda_device,
						args=OmegaConf.to_container(args))
