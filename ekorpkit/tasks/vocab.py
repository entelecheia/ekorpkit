...
def extract_nouns(**args):
    ...
    if prebuilt_dic_words:
        ...
        print(f'{len(prebuilt_dic_words)} words from the pre-built dictionary in {prebuilt_dic_dir} are loaded')

    status = [[f' {i} ', name, ' - ', ' - ', ' - ', Path(corpus_file).name] 
    ...
    if filter_only:
        ...
        noun_extractor = LRNounExtractor_v2(
            ...
            max_frequency_when_noun_is_eojeol=max(min_noun_frequency, min_eojeol_frequency),
        ...
        nouns = [{'word': word, 'frequency': n_score.frequency, 'score': n_score.score} 
    ...
    if stopwords_file:
        stopwords = set([w.strip().lower() for w in open(stopwords_file).read().split('\n') if len(w.strip()) > 1])
        print('Stopwords:')
        print(stopwords)
        print('-' * 80)
        if stopwords:
    ...
    for noun in nouns:
        ...
        if len(word) >= min_noun_length and noun['score'] >= min_keep_score :
            if (re.search(noun_pattern, word) and word[-1] != '들' 
    ...
    if addwords_file:
        addwords = set([w.strip().lower() for w in open(addwords_file).read().split('\n') if len(w.strip()) > 1])
        print('Add words:')
        print(addwords)
        print('-' * 80)
        if addwords:
