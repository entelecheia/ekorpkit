...
def info_docs(**args):
	...
	for info_path in info_paths:
		...
		info_output_path = Path(info_cfg.base_dir) / info_cfg.info_archive_dir / f'{info.name}.yaml'
		info_output_path.parent.mkdir(parents=True, exist_ok=True)
		info_src_path = Path(info_cfg.base_dir) / info_cfg.info_source_dir / f'{info.name}.yaml'
		...
		if os.path.exists(info_src_path):
			...
			for key, val in info_cfg.aggregate_info.items():
				if isinstance(info.splits, DictConfig):
					vals = [split[val] for split in info.splits.values() if val in split]
				else:
					vals = [split[val] for split in info.splits if val in split]
				info[key] = sum(vals)
			for key, value in info_cfg.modified_info.items():
				if isinstance(info.splits, DictConfig):
					vals = [get_modified_time(f'{data_file_dir}/{split[value]}') 
								for split in info.splits.values() if value in split]
				else:
					vals = [get_modified_time(f'{data_file_dir}/{split[value]}') 
								for split in info.splits if value in split]
			...
			doc_output_path = Path(info_cfg.base_dir) / info_cfg.info_output_dir / f'{info.name}.md'
		else:
			available = False
		...
		if available:
			...
			if sample_output_dir:
				save_sample_text(data_file_dir, sample_text_path, info_cfg.sample_max_lines)
			...
			if sample_output_dir:
				md_contents = markdown_template.format(name=fullname, src_link=src_link, sample_text_link=sample_text_link)
			else:
...
def make_table(infos, table_cfg):
	...
	if make_links:
		df[make_links.column] = df[make_links.column].apply(lambda x: make_links.link_format.format(x=x))
