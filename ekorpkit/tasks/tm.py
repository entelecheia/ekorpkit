...
def topic_model(**args):
    ...
    if 'train' in task_args:
        ...
        tm.train_model(model_type=args.model_type, save=True, 
            k=args.k, alpha=0.1, eta=0.01, tw=IDF, rm_top=5, 
            eval_coherence=False, set_word_prior=True, 
        ...
        # tm = TopicModel(model_name=model_name, model_dir=model_dir, output_dir=args.output_dir)
        tm.load_model(model_file=args.model_file)
        tm.infer_corpus(args.input_dir, args.corpus_names, args.export_dir, 
            compress=args.compress, num_workers=args.num_workers)
        ...
        # tm.load_model(model_file=args.model_file)
        tm.transform_topic_dists(args.input_dir, args.corpus_names, args.export_dir, compress=args.compress,
            column_evals=column_evals, aggregations=aggregations, groupby_cols=args.groupby_cols)

    else:
        raise ValueError(
            'Not found any proper subtask. Check the `subtask` argument')
